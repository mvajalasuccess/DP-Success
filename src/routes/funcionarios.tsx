...
function Employees() {
  ...
  async function createEmployee() {
    ...
    const bankMinutes = match ? (Number(match[1] === "-" ? -1 : 1) * (Number(match[2]) * 60 + Number(match[3]))) : 0;
    if (!form.name || !form.role || !form.department || !salary || !form.admission) { setError("Preencha nome, cargo, departamento, salário e admissão."); setSaving(false); return; }
    const dep = await supabase.from("departments").upsert({ name:form.department }, { onConflict:"name" }).select("id").single();
  ...
  const mins=(v:number)=>{const s=v<0?"-":"+";const a=Math.abs(v);return `${s}${String(Math.floor(a/60)).padStart(2,"0")}:${String(a%60).padStart(2,"0")}`};

  return <div className="min-h-screen bg-background">
    <header className="border-b bg-background px-6 py-4"><div className="mx-auto flex max-w-[1500px] items-center justify-between"><Link to="/" className="flex items-center gap-2 text-sm text-muted-foreground"><ArrowLeft className="h-4 w-4"/> Voltar ao dashboard</Link><div className="text-sm font-semibold">DP Success · Cadastros</div></div></header>
    <main className="mx-auto max-w-[1500px] px-6 py-7">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-sm font-medium text-primary">Cadastro base</p><h1 className="mt-1 text-3xl font-bold">Funcionários</h1><p className="mt-1 text-sm text-muted-foreground">Cadastro conectado ao Supabase.</p></div><button onClick={()=>setOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground"><Plus className="h-4 w-4"/> Novo funcionário</button></div>
      {error&&<div className="mt-4 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{error}</div>}
      <div className="mt-6 grid gap-4 sm:grid-cols-3">{[["Funcionários ativos",String(employees.filter(e=>e.active).length),UserRound],["Funcionários cadastrados",String(employees.length),BriefcaseBusiness],["Banco inicial lançado",employees.length?`${Math.round(employees.filter(e=>e.initial_bank_minutes!==0).length/employees.length*100)}%`:"0%",WalletCards]].map(([label,value,Icon])=><Card key={label as string} className="p-4"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon className="h-5 w-5"/></div><div><p className="text-xs text-muted-foreground">{label}</p><p className="text-xl font-bold">{value}</p></div></div></Card>)}</div>
