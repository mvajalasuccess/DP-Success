...
function competenceDates(referenceYear: number, referenceMonth: number) {
  const end = new Date(referenceYear, referenceMonth - 1, 20);
  const start = new Date(referenceYear, referenceMonth - 2, 21);
  return { start, end };
}

function periodName(p: Period) {
  return `Competência ${String(p.reference_month).padStart(2, "0")}/${p.reference_year}`;
}
...
export function PointClosing() {
  ...
  async function createCompetence() {
    ...
    setError("");
    const now = new Date();
    const referenceYear = now.getDate() >= 21 ? (now.getMonth() === 11 ? now.getFullYear() + 1 : now.getFullYear()) : now.getFullYear();
    const referenceMonth = now.getDate() >= 21 ? (now.getMonth() + 1) % 12 + 1 : now.getMonth() + 1;
    const payload = { reference_year: referenceYear, reference_month: referenceMonth, status: "aberto" };

    const { error } = await supabase.from("time_periods").insert(payload);
    if (error) setError(error.code === "23505" ? "Essa competência já está cadastrada." : error.message);
    else await load();
    setSaving(false);
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-[1500px] px-6 py-7">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div><p className="text-sm font-medium text-primary">Operação</p><h1 className="mt-1 text-3xl font-bold">Fechamento de Ponto</h1><p className="mt-1 text-sm text-muted-foreground">Controle as competências no ciclo do dia 21 ao dia 20.</p></div>
          <button disabled={saving} onClick={() => void createCompetence()} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground disabled:opacity-50"><Plus className="h-4 w-4" /> Nova competência</button>
        </div>
        {error && <div className="mt-4 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{error}</div>}
        <Card className="mt-6 p-5"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><CalendarDays className="h-5 w-5" /></div><div><p className="font-semibold">Regra atual de fechamento</p><p className="text-sm text-muted-foreground">Toda competência começa no dia 21 e termina no dia 20 do mês seguinte.</p></div></div><div className="rounded-lg border px-4 py-2 text-sm font-medium">21 → 20</div></div></Card>
        <div className="mt-6 space-y-3">
          {loading ? <Card className="p-8 text-center text-muted-foreground">Carregando competências...</Card> :
          periods.length === 0 ? <Card className="p-8 text-center text-muted-foreground">Nenhuma competência cadastrada.</Card> :
          periods.map((p) => { const {start,end}=competenceDates(p.reference_year,p.reference_month); return (
