    ...
    <div className="min-h-screen bg-background">
      <aside className="fixed inset-y-0 left-0 hidden w-60 border-r bg-sidebar lg:block">
        ...
        <nav className="space-y-1 p-3 text-sm">
          ...
          {["Funcionários","Departamentos","Cargos","Jornadas / Escalas","Fechamento de Ponto","Lançamentos","Banco de Horas","Atestados","Faltas e Ocorrências","Relatórios","Comparativos","KPIs","Configurações"].map(item=>item==="Funcionários" ? <Link key={item} to="/funcionarios" className="block rounded-lg px-3 py-2.5 text-sidebar-foreground/75 hover:bg-sidebar-accent">{item}</Link> : item==="Fechamento de Ponto" ? <Link key={item} to="/fechamento-ponto" className="block rounded-lg px-3 py-2.5 text-sidebar-foreground/75 hover:bg-sidebar-accent">{item}</Link> : item==="Banco de Horas" ? <Link key={item} to="/banco-horas" className="block rounded-lg px-3 py-2.5 text-sidebar-foreground/75 hover:bg-sidebar-accent">{item}</Link> : <div key={item} className="rounded-lg px-3 py-2.5 text-sidebar-foreground/75 hover:bg-sidebar-accent">{item}</div>)}
        </nav>
      </aside>
