...
function PointClosing() {
  ...
  async function load(){setLoading(true);const db=supabase as any;const {data,error}=await db.from("competencies").select("id,name,start_date,end_date,status,closed_at").order("end_date",{ascending:false});if(error)setError(error.message);else setPeriods(data??[]);setLoading(false);}
  useEffect(()=>{void load();},[]);

  async function createCompetence(){
    setSaving(true);setError("");const db=supabase as any;
