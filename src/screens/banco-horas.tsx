...
export function BankHours() {
  ...
  async function load() {
    ...
    const db=supabase;
    const {data: comps,error:ce}=await db.from("competencies").select("id,name,start_date,end_date,status").order("end_date",{ascending:false}).limit(1);
  ...
  const fmt=(v:number)=>{const s=v<0?"-":"+";const a=Math.abs(v);return `${s}${String(Math.floor(a/60)).padStart(2,"0")}:${String(a%60).padStart(2,"0")}`;};

  return <div className="min-h-screen bg-background"><header className="border-b px-6 py-4"><div className="mx-auto flex max-w-[1500px] justify-between"><a href="/" className="flex items-center gap-2 text-sm text-muted-foreground"><ArrowLeft className="h-4 w-4"/> Voltar</a><span className="font-semibold">DP Success · Banco de Horas</span></div></header>
  <main className="mx-auto max-w-[1500px] px-6 py-7"><p className="text-sm font-medium text-primary">Operação</p><h1 className="mt-1 text-3xl font-bold">Banco de Horas</h1><p className="mt-1 text-sm text-muted-foreground">Saldo inicial, movimentações da competência e saldo atualizado.</p>
  ...
  <Card className="mt-6 overflow-hidden"><div className="border-b p-5"><h2 className="font-display font-bold">Saldo por funcionário</h2><p className="text-xs text-muted-foreground">{competence?`${new Date(competence.start_date+"T12:00:00").toLocaleDateString("pt-BR")} → ${new Date(competence.end_date+"T12:00:00").toLocaleDateString("pt-BR")}`:"Nenhuma competência cadastrada"}</p></div>
  <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-muted/40 text-xs text-muted-foreground"><tr><th className="px-5 py-3">Funcionário</th><th className="px-5 py-3">Departamento</th><th className="px-5 py-3">Saldo anterior</th><th className="px-5 py-3">Movimentação</th><th className="px-5 py-3">Saldo atual</th></tr></thead><tbody className="divide-y">{rows.map(r=><tr key={r.employee_id}><td className="px-5 py-4 font-medium">{r.name}</td><td className="px-5 py-4 text-muted-foreground">{r.department}</td><td className="px-5 py-4">{fmt(r.opening)}</td><td className="px-5 py-4">{fmt(r.movement)}</td><td className={`px-5 py-4 font-bold ${r.current<0?"text-destructive":"text-primary"}`}>{fmt(r.current)}</td></tr>)}</tbody></table>{!loading&&rows.length===0&&<div className="p-8 text-center text-sm text-muted-foreground">Nenhum funcionário ativo encontrado.</div>}</div></Card></main></div>;
}